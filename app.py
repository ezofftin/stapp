...
# 배포시 유의점
# 생성된 requirements.txt에서 certifi 라인 삭제하기

# requirements.txt 파일 활용 : 이 파일을 이용하면 일일이 pip install 할 필요없음
...
# 선형회귀 기반  AR(5)모형 예측.
    ...
    ax.plot(ser, color = 'red', marker='o', linestyle ='--', linewidth=1.5, label = 'AR(5)')
    ax.legend(loc='best')    
...
with col1:
    lottie = loadJSON('code_06/lottie-stock-candle-loading.json')
...
# 주의: datetime.today()에는 항상 변하는 "시:분:초"가 들어 있어서 cache가 작동하지 않는다.
